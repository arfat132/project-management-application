...
const Category = ({ projects: allProjects, stage }) => {
	...
	useEffect(() => {
		const filteredProjects =
			allProjects?.map(search(searchString)).filter(filterProjectByStage(stage.toLowerCase())) || [];
    ...
    return (
        <div className='flex flex-col flex-shrink-0 w-72'>
				...
				<div
					className={`flex flex-col pb-2 overflow-auto scrollbar min-h ${isOver && 'bg-slate-800'}`}
					ref={drop}
