...
const Project = ({ project }) => {
    ...
    return (
        <div
            className={`relative flex flex-col items-start p-4 mt-3 text-white bg-slate-800 rounded-lg cursor-pointer bg-opacity-90 group hover:bg-opacity-100 ${match && 'border-2 border-blue-700'
            ...
            <div className='flex items-center w-full mt-3 text-xs font-medium text-gray-400'>
                <div className='flex items-center'>
                    ...
                    <span className='ml-1 leading-none'>{moment(createdAt).format('ll')}</span>
                </div>
                <img
